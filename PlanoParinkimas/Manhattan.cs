...
{
    public partial class Manhattan : Form
    {
        private DataTable duomenysFromTable;
        private DataGridView datagridViewFromFrom;

        public Manhattan(DataGridView duomenys, DataTable duomenys2)
        {
        ...
        private void ManhattanBetter_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < amountofrows; i++)
            {
                ...
                chart1.Series[seriesName].Sort(PointSortOrder.Descending, "Y");
                ...
                foreach (DataRow dr in sortedTable.Rows)
                {
                    try
                    {
                        if (String.Equals(seriesName, dr["Planas"].ToString(), StringComparison.Ordinal))
                        {
                            ...
                            yvals.Add(Convert.ToDecimal(dr["Manhattan"].ToString()));
                           ;
                        }
                    }
                    catch (Exception)
                ...
                try
                {
                    yvals = yvals.OrderBy(x => x).ToList();
                    xvals = xvals.OrderBy(x => x).ToList();
                    chart1.Series[seriesName].XValueType = ChartValueType.String;
                    chart1.Series[seriesName].YValueType = ChartValueType.Auto;
                    chart1.Series[seriesName].Points.DataBindXY(xvals, yvals);
                }
                catch (Exception)
                {
